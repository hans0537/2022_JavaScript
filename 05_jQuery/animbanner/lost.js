(function(){
    ...
    (function loopSea(){
        // 하나의(동일한 엘리먼트에) 추가적인 animate를 주면 순차적으로 진행
        $waterFront.animate({'bottom': '-65px', 'left': '-30px'}, 500)
                   .animate({'bottom': '-60px', 'left': '-25px'}, 500);
    
        $waterBack.animate({'bottom': '15px', 'left': '-20px'}, 500)
        ...
        $questionMark.delay(500).animate({'opacity': 1}, 1000, function(){ // $questionMark 등장
            $title1.animate({'opacity': 1}, 1000, function(){   // $title1 등장
                $title2.animate({'opacity': 1}, 1000, function(){   // $title2 등장
                    $form.animate({'left': '0px'}, 500);    // $form 등장
                }); // end $title2
            }); // end $title1
        }); // end $questionMark
    }); // end $boat
...
/*
    $.when($boat.animate({'left': '20px'}, 2000))
        ...
        .then(function(){$form.animate({'left': '0px'}, 500);});
*/

    // 방법3 : '단일 object' 애니메이션의 경우 promise() 만으로도 가능.
    // 즉       위 경우 $.when() 을 사용하지 않아도 됨
    ...
    .done(function(){$form.animate({'left': '0px'}, 500);});
