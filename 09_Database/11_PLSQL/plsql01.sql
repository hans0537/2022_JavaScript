...
DECLARE -- 선언부 
	...
	v_name VARCHAR(10);
BEGIN   -- 실행부
	SELECT 10 INTO v_age FROM dual; -- SELECT 결과를 변수에 값 대입
...
DECLARE
	...
	v_birthday t_emp2.birthday%TYPE;
BEGIN
	SELECT empno, name, birthday 
	INTO v_empno, v_name, v_birthday 
	FROM T_EMP2 
...
/* 예제  
 * 교수번호를 입력받은 수 professor 테이블을 조회하여 해당 교수의
...
DECLARE
	...
	v_hire t_professor.hiredate%TYPE;
BEGIN
	SELECT profno, name, deptno, hiredate 
	INTO v_profno, v_name, v_deptno, v_hire
	FROM t_professor 
	WHERE profno = '&profno';	-- 사용자에게 입력받아서 변수에 할당. &기호 사용
	DBMS_OUTPUT.PUT_LINE(v_profno || ' ' || v_name || ' ' || v_deptno || ' ' || v_hire);
END;
/
...
SET VERIFY OFF -- &를 이용한 치환변수의 값 변화 표시를 OFF 한다 (기본값 ON)
...
BEGIN
	...
	BEGIN
		DBMS_OUTPUT.PUT_LINE(v_first);	-- 안쪽 블록에선 바깥쪽 블럭에서 선언한 변수 사용 가능
